...
export default function Tag(props) {
  ...
  return (
    <div>
      {showTag ? (
        <div className="mr-2 mb-2 flex justify-center items-center justify-between w-auto h-6 rounded-[15px] py-0.5 border border-[#404040]">
          <TagText>{props.children}</TagText>
          <TagButton onClick={() => setShowTag(false)} />
        </div>
      ) : null}
    </div>
  );
...
export const TagText = (props) => {
  return (
    <label className="ml-2 text-[9px] font-bold text-white leading-5 tracking-[.21em] uppercase">
      {props.children}
    </label>
  );
};
...
export const TagButton = (props) => {
  return (
    <button className="mr-0.5" onClick={props.onClick}>
      <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
